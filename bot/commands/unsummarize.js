...
const command = {
    ...
    async execute(message, args) {
        ...
        if(err) throw err;
        if(docs.length == 0) return message.channel.send(`You are not subscribed to ${team.fullName}'s game summaries here!`).catch(console.error);

        // eslint-disable-next-line no-unused-vars
        let error, doc = await summaries.deleteOne({channel_id: message.channel.id, team:team.id});
        if(error) throw error;

        message.channel.send(`Unsubscribed this channel from ${team.fullName}'s game summaries here!!`).catch(console.error);
