const command = {
    ...
    async execute(message, args) {
        if(args.length){
            ...
            if(!command) return message.channel.send("There is no command with the name "+args[0]).then(global.client.messageFreq.mark()).catch(console.error);
            return message.channel.send(command.description).then(global.client.messageFreq.mark()).catch(console.error);
            ...
            return message.channel.send(commandList).then(global.client.messageFreq.mark()).catch(console.error);
        }
    },
};
