const command = {
    ...
    async execute(message, args) {
        if(args.length){
            ...
            if(!command) return message.channel.send("There is no command with the name "+args[0]).catch(console.error);
            return message.channel.send(command.description).catch(console.error);
            ...
            return message.channel.send(commandList).catch(console.error);
        }
    },
};
