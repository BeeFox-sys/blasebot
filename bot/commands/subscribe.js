...
const command = {
    ...
    async execute(message, args) {
        ...
        if(err) throw err;
        if(docs.length > 0) return message.channel.send("You already have subscibed this channel to a team, or this team to a channel! use bb!unsubscibre to remove the subscription").then(global.client.messageFreq.mark()).catch(console.error);
        ...
        return message.channel.send(`Subscribed this channel to the ${team.nickname}'s games!`).then(global.client.messageFreq.mark()).catch(console.error);
