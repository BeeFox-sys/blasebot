...
const command = {
    name: "forbidden",
    aliases: [],
    description: "Toggles if forbidden knowlage is disabled in this guild\nDefault: Hide forbidden knowlage\nbb!subscribe [team name]",
    async execute(message, args) {

        if(!message.guild) return message.channel.send("This command must be used in a guild!");
        if(!message.channel.permissionsFor(message.member).has("MANAGE_GUILD")) return message.channel.send("You require the manage server permission to run this command!");
