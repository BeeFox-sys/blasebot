...
const command = {
    ...
    async execute(message, args) {
        ...
        if(message.guild && !message.channel.permissionsFor(message.member).has("MANAGE_GUILD")) return message.channel.send("You require the manage server permission to run this command!").then(global.stats.messageFreq.mark()).catch(console.error);
        ...
        return await message.channel.send(`Now ${guild.forbidden?"showing":"hiding"} forbidden knowledge in this guild!`).then(global.stats.messageFreq.mark()).catch(console.error);
