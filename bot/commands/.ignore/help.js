...
const command = {
    ...
    async execute(message, args) {
        if(args.length){
            ...
            if(!command) return message.channel.send("There is no command with the name "+args[0]).catch(messageError);
            let aliases = Array.from(command.aliases);
            aliases.unshift(command.name);
            return message.channel.send(`Aliases: ${aliases.join(", ")}\n`+command.description).catch(messageError);

        } else {
            //Listing commands
            let commands = message.client.commands.filter(c=>!c.hide).map(c => c.name).filter((v,i,a)=>{if(a.indexOf(v)!==i)return false;else return true;}).sort();
            ...
            return message.channel.send(commandList).catch(messageError);
        }
    },
};
