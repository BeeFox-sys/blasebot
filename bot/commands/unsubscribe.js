...
const command = {
    ...
    async execute(message, args) {
        ...
        if(err) throw err;
        if(docs.length == 0) return message.channel.send("You are not subscribed to a team here!").then(global.stats.messageFreq.mark()).catch(console.error);

        // eslint-disable-next-line no-unused-vars
        let error, doc = await subscriptions.deleteOne({channel_id: message.channel.id});
        if(error) throw error;

        message.channel.send("Unsubscribed this channel from any games!").then(global.stats.messageFreq.mark()).catch(console.error);
