...
const command = {
    ...
    async execute(message, args) {
        ...
        if(err) throw err;
        if(docs.length > 0) return message.channel.send("You already have subscibed to this teams summaries! use bb!unsummarize to remove the summaries").then(global.stats.messageFreq.mark()).catch(console.error);
        ...
        return message.channel.send(`Subscribed this channel to the ${team.nickname}'s game summaries!`).then(global.stats.messageFreq.mark()).catch(console.error);
    },
};
